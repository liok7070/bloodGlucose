...
"""Library for the AS7263 Near Infrared Sensor."""
"""Adapted from as7262 __init__ and __main__"""
import time
import struct
...
class as7263VirtualRegisterBus():
    """AS7263 Virtual Register.

    This class implements the wacky virtual register setup
    of the AS7263 annd allows i2cdevice.Device to "just work"
    ...
    def read_i2c_block_data(self, address, register, length):
        """Read one or more values from AS7263 virtual registers."""
        result = []
...
class FWVersionAdapter(Adapter):
    """Convert the AS7263 firmware version number to a human-readable string."""
...
class AS7263:
    def __init__(self, i2c_dev=None):
        self._as7263 = Device(0x49, i2c_dev=as7263VirtualRegisterBus(i2c_dev=i2c_dev), bit_width=8, registers=(
            ...
            Register('DATA', 0x08, fields=(
                BitField('r', 0xFFFF00000000000000000000),
                BitField('s', 0x0000FFFF0000000000000000),
                BitField('t', 0x00000000FFFF000000000000),
                BitField('u', 0x000000000000FFFF00000000),
                BitField('v', 0x0000000000000000FFFF0000),
                BitField('w', 0x00000000000000000000FFFF),
            ), bit_width=96),
            Register('CALIBRATED_DATA', 0x14, fields=(
                BitField('r', 0xFFFFFFFF << (32 * 5), adapter=FloatAdapter()),
                BitField('s', 0xFFFFFFFF << (32 * 4), adapter=FloatAdapter()),
                BitField('t', 0xFFFFFFFF << (32 * 3), adapter=FloatAdapter()),
                BitField('u', 0xFFFFFFFF << (32 * 2), adapter=FloatAdapter()),
                BitField('v', 0xFFFFFFFF << (32 * 1), adapter=FloatAdapter()),
                BitField('w', 0xFFFFFFFF << (32 * 0), adapter=FloatAdapter()),
            ), bit_width=192),
        ))
        ...
        for register in self._as7263.registers:
            register = self._as7263.registers[register]
            for field in register.fields:
                ...
                if isinstance(field.adapter, LookupAdapter):
                    for key in field.adapter.lookup_table:
                        ...
                        name = 'AS7263_{register}_{field}_{key}'.format(
    ...
    def soft_reset(self):
        """Set the soft reset register bit of the AS7263."""
        self._as7263.set('CONTROL', reset=1)
    ...
    def get_calibrated_values(self, timeout=10):
        ...
        t_start = time.time()
        while self._as7263.get('CONTROL').data_ready == 0 and (time.time() - t_start) <= timeout:
            pass
        data = self._as7263.get('CALIBRATED_DATA')
        return CalibratedValues(data.w, data.v, data.u, data.t, data.s, data.r)

    def set_gain(self, gain):
        """Set the gain amount of the AS7263.

        :param gain: gain multiplier, one of 1, 3.7, 16 or 64

        """
        self._as7263.set('CONTROL', gain_x=gain)

    def set_measurement_mode(self, mode):
        """Set the AS7263 measurement mode.

        :param mode: 0-3

        """
        self._as7263.set('CONTROL', measurement_mode=mode)

    def set_integration_time(self, time_ms):
        """Set the AS7263 sensor integration time in milliseconds.

        :param time_ms: Time in milliseconds from 0 to ~91

        """
        self._as7263.set('INTEGRATION_TIME', ms=time_ms)

    def set_illumination_led_current(self, current):
        """Set the AS7263 illumination LED current in milliamps.

        :param current: Value in milliamps, one of 12.5, 25, 50 or 100

        """
        self._as7263.set('LED_CONTROL', illumination_current_limit_ma=current)

    def set_indicator_led_current(self, current):
        """Set the AS7263 indicator LED current in milliamps.

        :param current: Value in milliamps, one of 1, 2, 4 or 8

        """
        self._as7263.set('LED_CONTROL', indicator_current_limit_ma=current)

    def set_illumination_led(self, state):
        """Set the AS7263 illumination LED state.

        :param state: True = On, False = Off

        """
        self._as7263.set('LED_CONTROL', illumination_enable=state)

    def set_indicator_led(self, state):
        """Set the AS7263 indicator LED state.

        :param state: True = On, False = Off

        """
        self._as7263.set('LED_CONTROL', indicator_enable=state)

    def get_version(self):
        """Get the hardware type, version and firmware version from the AS7263."""
        version = self._as7263.get('VERSION')
